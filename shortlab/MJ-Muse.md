...
5. **给出参数**：默认 V8.2、stylize 300、chaos 0、raw 关闭。风格预设可覆盖默认值，用户明确指定的参数优先级最高。
6. **复核事实**：确认没有新增人物、对白、道具、品牌、符号、文字、动作或人物关系。
...
用户要求可直接粘贴的命令时，才把参数编译为单行 flags。API 或客户端由用户自行配置；本文件只提供中立的提示词与参数结果。
