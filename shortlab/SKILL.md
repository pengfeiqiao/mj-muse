---
name: mj-muse
description: 将用户的视觉意图改写为 Midjourney 提示词，提供 84 种风格、参数策略、事实锁与参考图分工。适用于 Midjourney 风格选择、提示词润色、参数建议和提示词审查。
---
...
5. **执行长度闸门**：用于提交生成的每条完整提示词不得超过 1000 个字符；目标控制在 800–900 个字符，为参数和用户追加内容保留余量。
...
- **安全目标**：先把正文控制在约 800–900 字符；需要同栏拼接参数或参考图 URL 时，为它们预留实际长度。
...
```text
Style: 风格名称 (style-id)
Prompt: final English visual prompt (完整提交内容不超过 1000 字符)
