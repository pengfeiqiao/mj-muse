...
ShotLab 的 Midjourney 输入存在 **1000 个 Unicode 字符硬上限**。任何准备交给模型的单条完整提示词都必须先通过长度检查；超过 1000 字符时不得提交，也不得机械截断，必须在保留事实锁的前提下重写压缩。这个约束优先于风格丰富度、示例完整度和其他所有提示词规则。

- 常规目标：完整提交文本控制在 **650–850 字符**。
- 同一输入框还要拼接参数或参考图 URL 时：正文通常控制在 **500–700 字符**，并按实际 URL 长度继续缩短。
- 多套方案逐条独立计数，不允许用“平均长度”代替逐条校验。
- 无法可靠计数时，使用更短版本，不要贴近 1000 字符边界。
...
5. **执行长度闸门**：用于提交生成的每条完整提示词不得超过 1000 个字符；常规目标为 650–850 个字符，同栏含参数或 URL 时进一步缩短。
...
- **安全目标**：完整提交文本优先控制在 650–850 字符；需要同栏拼接参数或参考图 URL 时，正文通常控制在 500–700 字符，并为实际参数与 URL 预留长度。
...
- **输出前校验**：生成后计算字符数；超过 1000 就继续压缩，直到通过。无法在不损害事实锁的情况下压缩时，先向用户说明冲突，不提交超长提示词。
...
```text
Style: 风格名称 (style-id)
Prompt: final English visual prompt (完整提交内容必须不超过 1000 字符，建议 650–850)
...
`Length` 统计实际会提交给模型的完整文本，不统计 `Style:` 等说明标签。用户要求可直接粘贴的命令时，才把参数编译为单行 flags，并对合并后的整行重新计数。API 或客户端由用户自行配置；本文件只提供中立的提示词与参数结果。
